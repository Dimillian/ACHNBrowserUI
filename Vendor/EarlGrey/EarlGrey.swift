...
// PSPDFKit: made non-private
func GREYWaitUntilIdle() {
    GREYUIThreadExecutor.sharedInstance().drainUntilIdle()
}

func GREYWaitUntilIdleForced() {
    // This forces a runloop to execute the sync block,
    // where other idle calls has a fast-path that doesn't loop at all.
    // The fast path can miss operations that are queued but not yet tracked by the busy resource tracker.
    // This is a hack to make EarlGrey faster; vanilla always spins a runloop to prevent such issues.
    GREYUIThreadExecutor.sharedInstance().executeSync({}, error: nil)
}
