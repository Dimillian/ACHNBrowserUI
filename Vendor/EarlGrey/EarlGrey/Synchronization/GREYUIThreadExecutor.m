 ...
 *  PSPDFKit: changed from 0.1
 */
static const CFTimeInterval kMaximumSynchronizationSleepInterval = 0.001;
...
- (void)drainUntilIdle {
  // PSPDFKit: Performance fast path
  if ([self grey_areAllResourcesIdle]) { return; }
...
- (BOOL)drainUntilIdleWithTimeout:(CFTimeInterval)seconds {
  // PSPDFKit: Performance fast path
  if ([self grey_areAllResourcesIdle]) { return YES; }
