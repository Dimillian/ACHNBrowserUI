...
static BOOL GREYAllowCheapVisibilityCheckForElement(id element) {
    // PSPDFKit: Add elements that have weird frames to use the smart tap logic.
    // UISwipeActionStandardButton has a huge width on iOS 11.
    return !GREY_CONFIG_BOOL(GREYConfigKeyStrongVisibilityCheckEnabled) &&
    ![element isKindOfClass:NSClassFromString(@"UISwipeActionStandardButton")];
}
...
+ (CGRect)rectEnclosingVisibleAreaOfElement:(id)element {
  ...
  if (viewIntersectsScreen) {
      if (GREYAllowCheapVisibilityCheckForElement(element)) {
          visibleAreaRect = [view accessibilityFrame];
...
+ (CGPoint)visibleInteractionPointForElement:(id)element {
  ...
  if (GREYAllowCheapVisibilityCheckForElement(element)) {
      CGPoint interactionPointInFixedPoints = [element accessibilityActivationPoint];
...
+ (double)grey_percentViewVisibleOnScreen:(UIView *)view
  ...
  if (viewIntersectsScreen) {
      ...
      // HACK for performance
      if (GREYAllowCheapVisibilityCheckForElement(view)) {
          return 1.0;
      }
