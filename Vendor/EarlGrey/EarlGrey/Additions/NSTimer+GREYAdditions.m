...
+ (NSTimer *)greyswizzled_scheduledTimerWithTimeInterval:(NSTimeInterval)interval
  ...
  // PSPDFKit performance (ignore system gesture timeout)
  BOOL skipSelector = PSPDF_UNDECLARED_SELECTOR(aSelector == @selector(_timeOut);)

  if (!repeats && !skipSelector && GREY_CONFIG_DOUBLE(kGREYConfigKeyNSTimerMaxTrackableInterval) >= interval) {
    [GREYNSTimerIdlingResource trackTimer:timer
                                     name:[NSString stringWithFormat:@"Tracking Timer %@", timer]
                             removeOnIdle:YES];
  }
