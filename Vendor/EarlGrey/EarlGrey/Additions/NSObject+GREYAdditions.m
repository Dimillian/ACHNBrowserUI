...
- (void)greyswizzled_performSelector:(SEL)aSelector
  ...
  if ([NSThread isMainThread]) {
    ...
    // PSPDFKit: performance
    BOOL selectorIsSkippable = PSPDF_UNDECLARED_SELECTOR(aSelector == @selector(_hideScrollIndicators);)
    ...
    if (!selectorIsSkippable && maxDelayToTrack >= delay) {
